...
- **清理仓库死产物与冗余文件**：
  - 从版本控制中彻底移除了遗留的历史 tarball 产物包（`dsh-searxng-web-0.6.0.tgz`、`dsh-searxng-web-0.8.0.tgz`）以及冗余的 `package-lock.json`（统一使用 pnpm）。
- **文档与 0.1.5-rc.2 同步**：
...
- **纯 TypeScript 架构重构（零 JavaScript 残留）**：
  - 遵循 DeepSeek Harness 官方插件开发规范进行模块化纯 TypeScript 重构；
  - 移除了所有旧的 `.mjs` / `.js` 文件；
  - 测试套件转为纯 TypeScript（`test/standalone-test.test.ts`），由 Node `--experimental-strip-types` 原生运行；
  - 强化 SSRF 主机防护机制，在发起网络请求前校验并拒绝 localhost、环回、私网和保留地址。

...
export function isPrivateIPv4(ip) {
    ...
    if (o[0] === 0 || o[0] === 10 || o[0] === 127)
        return true; // current network, RFC 1918, loopback
    if (o[0] === 169 && o[1] === 254)
    ...
    if (o[0] >= 224)
        return true; // multicast and reserved (class D & E)
    return false;
}
...
export function isPrivateIPv6(ip) {
    ...
    if (match && match[1])
        return isPrivateIPv4(match[1]); // IPv4-mapped
    return false;
}
 ...
 */
export async function resolveFetchTarget(urlStr, guard) {
    let url;
    try {
        url = new URL(urlStr);
    }

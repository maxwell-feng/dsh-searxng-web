...
export function apply(ctx, config = {}) {
        ...
        if (Array.isArray(config.baseUrls)) {
            for (const item of config.baseUrls) {
                if (typeof item === "string" && item.trim())
                    list.push(normalizeBaseUrl(item.trim()));
            }
        }
    ...
    const hasHeader = (headerName) => Object.keys(instanceHeaders).some((h) => h.toLowerCase() === headerName.toLowerCase());
    ...
    ctx.logger?.info?.(`[searxng-web] providers registered (${endpoints.length} endpoint(s); primary: ${endpoints[0]})`);
}
